...
pi-remote是一个在 本地 pi/codex/claude等agent具备操作远程homelab或者vps能力的服务，不依赖于rdp、ssh等正向连接服务。即时目标机器没有公网ip并且在有防火墙的情况下，也能实现远程连接。
...
`pi-remote-cli` 可供任意支持命令行或标准 stdio MCP 的 Agent 使用。
...
export PI_REMOTE_TARGET=remote # 可选；remote 表示运行 Gateway 的机器
```

### CLI

适用于 Agent 通过 Shell 调用 `pi-remote-cli` 的场景。先将仓库中的`/skills/pi-remote-cli/`安装到 Agent 会扫描的 Skill 目录。

然后在 Agent 对话中显式调用 Skill，进行连接检查：

```text
$pi-remote-cli 在远程机器上执行id，返回命令结果
```
